...
type RestOf<T extends IComment | NewComment> = Omit<T, 'postedDate'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      postedDate: comment.postedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      postedDate: restComment.postedDate ? dayjs(restComment.postedDate) : undefined,
    };
  }
